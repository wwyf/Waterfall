...
MVVM架构主要由四部分组成，分别是Model(模型)、View(视图)和View Model(视图模型，通常包含绑定器)。

其中模型部分为数据访问层即后端数据，视图部分为前端图形用户界面，视图模型部分与视图层建立数据绑定，同时与模型层进行交互，将用户所需的数据反映成图形界面，同时将用户行为反映为后端数据变更。在本项目中，视图模型部分的功能由Vue.js提供。
<!-- 看看加不加这段：MVVM的视图模型是一个值转换器，这意味着视图模型负责从模型中暴露（转换）数据对象，以便轻松管理和呈现对象。 -->
<!-- 也看看加不加这段：该架构有助于将图形用户界面的开发与业务逻辑(后端逻辑)的开发分离开来。 -->

MVC架构主要由Model(模型)、View(视图)和Controller(控制器)组成。

在我们的项目中，后端模型部分为储存在数据库中的数据，视图部分交给前端进行处理，而控制器负责与前端的视图模型进行交互，将后端数据同步至前端模型。
...
1. 用户表：包含用户称呼、用户名、密码、公司信息、身份信息、额度信息、用户状态等。
1. 订单表：包含订单编号、订单类型、订单金额、订单详情、订单状态等。
1. 资金流动表：包含记录编号、金额数目、关联订单、资金流向、资金状态等。
