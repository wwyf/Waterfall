...
>>>>>>> cd8ad8b4c4a7dabdb8094dd0eff05d4ac948ec6f
...
TODO: 这个部分我还没想好, 初步想法是平台自动把已注册还没被审核的订购商
推送给管理员进行审核, 然后管理员就写一个"通过/不通过"到数据库里面即可
...
TODO: 这个部分和1.3.5一致, 可以复制, 把名字换一下

##### 1.3.7 生产资格认证

TODO: 这个部分主要是微供应商提交审核请求, 和1.3.6配套
...
TODO: 这个部分的过程是绑定银行卡, 并让微供应商象征性支付1元测试是否可用
然后把银行卡帐号存到资金系统数据库里面, 微供应商收订金和最后尾款都通过银行卡, 
没有通过绑定的不可以申请供应额度

##### 1.3.10 申请供应额度

TODO: 系统预先计算一个额度, 然后可以逐渐提升, 供应商可以使用不超过总额度的
供应额度并生产, 如果供应商交付的产品数量不符合要求, 或质量很差, 会使额度减少
并被处以违约金 

##### 1.3.11 查看派发子订单

TODO: 一个微供应商可以有多个子订单, 对应每次申请的供应额度, 微供应商可以查看订单
的大小, 期限等数据, 同时会定期收到系统的完成提醒

##### 1.3.12 取消子订单

TODO: 一个微供应商可以取消订单, 但会使额度减少并按已完成比例和进行时间处以违约金

##### 1.3.13 提交完成子订单

TODO: 一个微供应商可以分批完成每一个子订单, 并申请物流配送, 
子订单需要多次提交并更新完成情况, 子订单完成度100%时会收到尾款, 开始时会收到订金
...
3) 事件流
       ![](figure/usecase/uc_sub/code4_11-13.png)
...
3) 事件流
        ![](figure/usecase/uc_sub/uc_client_cancel.png)
...
3) 事件流
    ![](figure/usecase/uc_sub/uc_client_commit.png)
