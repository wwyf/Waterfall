...
### 2.1 架构描述
<!-- TODO:[hgl] -->
项目前端(Browser)采用MVVM的软件架构，后端(Server)采用MVC的软件架构。

在前端(Browser)中：

MVVM架构主要由四层组成，分别是Model(模型)、View(视图)和View Model(视图模型，通常包含绑定器)。

Model层也称模型层(数据访问层)，即后端数据。
View层也称视图层，为前端图形用户界面。
View Model层也称视图模型层，与视图层建立数据绑定，同时与模型层进行交互，将用户所需的数据反映成图形界面，同时将用户行为反映为后端数据变更。在本项目中，视图模型层的主要功能由Vue.js框架提供。
...
1. User Model，又称用户表：包含用户称呼、用户名、密码、公司信息、身份信息、额度信息、用户状态等。
2. Order Model，又称订单表：包含订单编号、订单类型、订单金额、订单详情、订单状态等。
3. Account Model，又称账户表：包含账户代号、账户金额，账户信息，账户状态等。

![](figure/key_abstraction_new.png)
