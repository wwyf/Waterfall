...
TODO:[hgl]
项目采用MVVM的软件架构。
MVVM架构主要由四部分组成，分别是Model(模型)、View(视图)和View Model(视图模型)。
其中模型部分为数据访问层即后端数据，视图部分为前端图形用户界面，而视图模型(包括绑定器)由使用的框架Vue.js提供，该框架能够连接界面与数据，将用户所需的数据反映成图形界面，同时将用户行为反映为后端数据变更。
